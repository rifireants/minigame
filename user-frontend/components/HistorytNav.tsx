'use client'

import Link from "next/link";
import { BsDice6, BsHouse, BsWallet2 } from 'react-icons/bs';

export default function HistorytList() {
  return (
    <div className="grid grid-cols-3 gap-2">
      <Link
        href="/payment"
        className="w-full inline-flex justify-center items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-3 rounded-md transition"
      >
        <BsWallet2 size={16} />
        <span>충전/출금</span>
      </Link>

      <Link
        href="/game"
        className="w-full inline-flex justify-center items-center gap-1 border border-blue-600 text-blue-600 hover:bg-blue-50 text-sm font-semibold py-2 px-3 rounded-md transition"
      >
        <BsDice6 size={16} />
        <span>게임</span>
      </Link>

      <a
        href="/"
        target="_blank"
        rel="noopener noreferrer"
        className="w-full inline-flex justify-center items-center gap-1 border border-gray-600 text-gray-700 hover:bg-gray-100 text-sm font-semibold py-2 px-3 rounded-md transition"
      >
        <BsHouse size={16} />
        <span>홈</span>
      </a>
    </div>
  );
}
